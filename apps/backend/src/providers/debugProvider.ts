...
vscode.debug.onDidReceiveDebugSessionCustomEvent(e => {
  if (e.event === 'output') {
    pubsub.publish('DEBUG_EVENT', { debuggerEvent: { event: 'output', body: (e as any).body?.output ?? '' } });
  }
});
...
export const getDebugProvider = () => ({
  ...
  Mutation: {
    ...
    stepOver: () => { throw new Error('Step Over not implemented.'); },
    stepInto: () => { throw new Error('Step Into not implemented.'); },
  },
});
