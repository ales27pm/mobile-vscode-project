...
jest.mock('http', () => require('../../__mocks__/http'), { virtual: true });
...
jest.mock(
  '../ui/uiState',
  () => ({
    setServerUiState: jest.fn(),
    getServerUiState: jest.fn(() => ({ status: 'inactive', lastUpdated: Date.now() })),
    onDidChangeServerUiState: jest.fn()
  }),
  { virtual: true }
);

jest.mock('../ui/logger', () => ({ logInfo: jest.fn(), logWarn: jest.fn(), logError: jest.fn() }), { virtual: true });
